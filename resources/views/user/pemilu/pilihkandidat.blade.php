...
<div class="min-h-screen bg-gray-100 flex flex-col items-center py-12 px-4 sm:px-6 lg:px-8">
    ...
    @if (session('success'))
        <div class="mb-6 w-full max-w-3xl bg-green-100 border border-green-400 text-green-800 px-4 py-3 rounded-lg text-center shadow-md">
            ✅ {{ session('success') }}
        </div>
    @endif

    @if (session('error'))
        <div class="mb-6 w-full max-w-3xl bg-red-100 border border-red-400 text-red-800 px-4 py-3 rounded-lg text-center shadow-md">
            ⚠️ {{ session('error') }}
        </div>
    @endif
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full max-w-6xl justify-center">
        ...
        @foreach ($kandidat as $kandi)
            {{-- Tampilkan kandidat sesuai sekolah user, kecuali guru bisa lihat semua --}}
...
<script>
    function toggleVisiMisi(button) {
        ...
        button.textContent = visiMisiDiv.classList.contains("hidden") 
            ? "📜 Tampilkan Visi Misi" 
            : "🔽 Sembunyikan Visi Misi";
    }
</script>
